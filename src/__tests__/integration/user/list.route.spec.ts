...
import { mockedLoginAdmRequest, mockedLoginNotAdmRequest, mockedUserAdmRequest, mockedUserNotAdmRequest } from "../../mocks";
...
describe("List user route tests", () => {
    ...
    it("Must not be able to list all users without Admin permission",async () => {
        ...
        const loginResponse = await request(app).post("/login").send(mockedLoginNotAdmRequest);
        const response = await request(app).get(baseUrl).set("Authorization", `Bearer ${loginResponse.body.token}`)
