...
describe("Update user route tests", () => {
  ...
  it("Should not be able update user without authentication" , async() => {
    ...
    const response = await request(app).patch(`/users/${user.body.id}`).send(newValues);
  ...
  it("Should not be able to update user with invalid id" , async() => {
    ...
    const response = await request(app).patch(`/users/4e99808c-c06d-4109-9b95-1a2fef3f8ea7`).send(newValues).set("Authorization",`Bearer ${userLogin.body.token}`)
  ...
  it("Should be able to update your own user" , async() => {
    ...
    const userLogin = await request(app).post("/login").send(mockedLoginNotAdmRequest);    
    const response = await request(app).patch(`/users/${user.body.id}`).send(newValues).set("Authorization",`Bearer ${userLogin.body.token}`)
